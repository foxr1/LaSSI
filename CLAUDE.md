...
**Engine semantics (2026-06; established by the DatagramDB investigation — full report: `/Users/fox/CLionProjects/datagram-db-mac/REPORT_engine_semantics.md`; prototype with `eval/_gsm_probe.py <case> [query_file]`, which runs any query over a cached `catabolites/<case>/gsmDB.txt` and matches the pipeline's DatagramDB stage byte-identically):**
- **Single pass, no fixpoint, no re-matching**: morphisms are computed once against the original graph; one rule's output is never visible to another rule's *pattern* in the same run (where-guards, however, evaluate against the mutated state).
- **The grammar file format is a booby-trap**: rules are `;`-SEPARATED (every rule but the last ends `;`, the last is bare) and the parser has no EOF — anything after the last parseable rule is *silently ignored*. A naively appended rule therefore never parses. To append: newline + `;` (terminating the old last rule) + the new rule, bare, last. Always confirm the rule parsed (it must appear in a `GSM_TRACE` fire-count CSV) before interpreting results.
- **Rule order in the file is irrelevant**: scheduling rank comes from variable-NAME collisions across rules (`sortVL`), cyclic and arbitrary for the live grammar. A rule with entirely fresh node/edge variable names gets rank 0 and runs before everything.
- **Return the entrypoint variable**: a trailing `(X)` where X ≠ the pattern's entrypoint registers a graph-wide replacement map entry (entrypoint → X, last matched row wins) that re-points ALL references including unrelated outer-context edges. (`normalise_acl` has this hazard whenever it fires; `fix_garden_path` is safe.)
- **Output is DAG-ified**: among rewrite-touched objects, edges that don't point strictly *down* the containment hierarchy are silently dropped at serialisation — a new upward edge survives only if the old downward containment is severed (`del`) in the same rule body.
- **The p3/p4 kernel-building family is starved by its own guards** (`unmatched` consults other rules' *match* tables, computed pre-mutation; where-guards then fail against the p1/p2-mutated state): across traced cases p3/p3simple/p3noobj/p3expl/p3fauxcop/implication/passivity apply **zero** times. The graph reaches the Python kernel builder shaped essentially by p1/p2/p0/docop only. (`implication`/`passivity` also reference a non-existent pattern `fauxcop` — they are dead rules.)

**New-rule authoring contract**: separator invariant (above) + fresh variable names + return the entrypoint + new edges must point down the containment (or sever the old containment) + verify with `eval/_gsm_probe.py` and `GSM_TRACE` fire counts against the cases the rule should AND should not affect, then a full per-case rebuild (`regen_gold.sh <case>`) — GSM edits invalidate `datagramdb_output.json` and everything downstream.
